 ...
 * `Authorization: Bearer ` header is attached to the request.
 *
...
function isRequest(input): input is Request {
...
function isUrl(input): input is URL {
  return typeof URL !== "undefined" && input instanceof URL;
}
function applyBaseUrl(input): RequestInfo | URL {
...
function mergeHeaders(...sources): Headers {
...
function getMediaType(headers) | null {
...
function getStringField(value, key) | undefined {
...
function inferResponseType(response): "json" | "text" | "blob" {
  const mediaType = getMediaType(response.headers);
...
export async function customFetch(
  ...
  const { responseType = "auto", headers, ...init } = options;
